...
export class Allenlund extends SearchSite {
  ...
  protected async search(task: ITASK) {
    ...
    await page.select('select[name="equipment_type"]', task.criteria.equipment.substr(0, 1).toUpperCase()); // V 表示 Van, R 表示 Reefer
  ...
  private async getDataFromHtml(): Promise<Array<IResultHTMLData>> {
    const data: Array<IResultHTMLData> = await this.page.evaluate(() => {
      ...
      trs.forEach((tr) => {
        let [, postingID, date, equipment, originCity, originState, destinationCity, destinationState, contact] =
