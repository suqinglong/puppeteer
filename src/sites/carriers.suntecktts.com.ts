...
export class CarriersSunteckttsCom extends SearchSite {
  ...
  public async search(task: ITASK) {
    try {
      ...
      await this.page.evaluate((pickUpDate: string) => {
        (document.querySelector('#load_board_search_shipDateStart') as HTMLInputElement).value = pickUpDate
      }, pickUpDate).catch(e => {
      ...
      Promise.race([
        this.page.waitForSelector('#js-load-board-results tbody tr[role=row]', {
          timeout: 10000
        }).then(() => 'hasData').catch(e => {
          this.log.log('warning: fail to waitForSelector #js-load-board-results tbody tr[role=row]')
        }),
        this.page.waitForSelector('#js-load-board-results .dataTables_empty', {
          timeout: 10000
        }).then(() => 'noData')
      ]).then(raceResult => {
      ...
      const resultHtml = await this.page.$eval('#js-load-board-results', input => input.outerHTML)
  ...
  private getDataFromHtml($: CheerioStatic, task: ITASK): Array<any> {
    ...
    $('tbody tr[role=row]').each((_number, element) => {
      let [
        date, method, equipment, equipmentSize, shipCity,
        sst, destCity, dest, delivDate, stops, pieces, weight,
        carrRate, miles, contact, phone, email, load] = Array.from($(element).find('td').map((_index, el) => {
          return $(el).text();
        }))

      result.push({
        date, method, equipment, equipmentSize,
        origin: [shipCity, sst].join(','),
        destination: [destCity, dest].join(','),
        delivDate, stops, pieces, weight,
        carrRate, miles, contact, phone, email, load
      })
