...
export class ConnectCoyoteCom extends SearchSite {
    ...
    protected async search(task: ITASK) {
        ...
        await (this.page as any)._client.send('Page.setDownloadBehavior', { behavior: 'allow', downloadPath: this.downloadPath });
        ...
        if (filePath) {
            await PostSearchData(ModifyPostData(task, this.getDataFromXlsx(filePath as string))).then((res: any) => {
    ...
    private getDataFromXlsx(filePath: string): Array<IResultHTMLData> {
        ...
        data.shift();
        data.forEach(item => {
            const date =  item[6] && item[6].match(/\d+\/\d+\/\d+/) && item[6].match(/\d+\/\d+\/\d+/)[0]
            ...
            result.push({
                date, origin, destination, origin_radius, destination_radius,
                'Load ID': item[0],
                'Mode': item[1],
                'equipment': item[2],
